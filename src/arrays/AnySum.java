...
/*
THIS SHOULD WORK FOR ANY SUM , change findSum( ) method to have count 3 or 4 for 3Sum or 4Sum
...
public class AnySum {
	...
	public static void main(String[] args) {
		...
		int whichSum = 2 ; // for foursum use 4 and for 3Sum use 3
		int target = 0;// target sum to be , 0 ,1 ,2 or any number 
    ...
    private List<List<Integer>> findSum(int[] nums, int target,int count  ,int start) {
    	...
    	for(int i = start; i <= nums.length - count; i ++) {
    		...
    		if(i > start && nums[i] == nums[i - 1]) { // handle duplicates
    			continue;
    		}
